    ...
    try:
        for key, data in OtsUidPathTable.scan(row_start=row_s, row_stop=row_e):
            ...
            for i in range(0, step):
                r_start = filenameHash + str(TimeEnd-3600*(step-i)) + '00000'
                r_stop = filenameHash + str(TimeEnd-3600*(step-i-1)) + '99999'
                nread = 0
                read_bytes = 0
                nwrite = 0
                write_bytes = 0
                ...
                for _key, _data in PathOtsUidTable.scan(row_start=r_start, row_stop=r_stop):
                    ...
                    try:
                        ...
                        continue
                    nread += int(log_list[10].split('=')[1])
                    #string = log_list[10]
                    nwrite += int(log_list[11].split('=')[1])
                    #string = log_list[11]
                    read_bytes += float(log_list[18].split('=')[1])
                    #string = log_list[18]
                    write_bytes += float(log_list[-5].split('=')[1])
                    #string = log_list[-5]
                    nseek += float(log_list[8].split('=')[1])
                    #string = log_list[8]
                    nseek += float(log_list[9].split('=')[1])
                    #string = log_list[9]
                    nseek += float(log_list[12].split('=')[1])
                    #string = log_list[12]
                    nseek += float(log_list[13].split('=')[1])
                    #string = log_list[13]
                    seek_bytes += float(log_list[25].split('=')[1])
                    #string = log_list[25]
                    seek_bytes += float(log_list[34].split('=')[1])
                    #string = log_list[34]
                    seek_bytes += float(log_list[35].split('=')[1])
                    #string = log_list[35]
                    seek_bytes += float(log_list[36].split('=')[1])
                    #string = log_list[36]
                    if index==1:
                        osize = float(log_list[14].split('=')[1])
                        #string = log_list[14]
                    csize = float(log_list[1].split('=')[1])
                    #string = log_list[1]
                SumAccessNum += index
                _list = [nread, read_bytes, nwrite, write_bytes, osize, csize, nseek, seek_bytes]
...
def main():
    ...
    dataset = DataSet(features=features, labels=labels, train_time_begin=9999999999.99 - 1523808000.00,
                                    ...
                                    StartKey=None, ForceBalance=False, capacity=10000, limit=1000)
